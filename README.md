...
- Gestão (Fase 3): receitas, produção e relatório como acima; **`GET /api/v1/inventory-items`** (insumos para o painel); **`GET /api/v1/me`** inclui `store_slug` e `store_name` (link vitrine no painel)
- Frontend: `http://localhost:3000` — `/loja/[slug]` vitrine; `/painel` (resumo, link vitrine), `/painel/pedidos` (lista + detalhe e **estado**), `/painel/receitas` (lista + **Produzir lote**), `/painel/receitas/nova`, `/painel/relatorio` (métricas + CSV); `/login`. **WhatsApp (vitrine):** em `stores.theme` use `{"vitrine": {"whatsapp": "+5511…"}}` (ou ajuste via SQL) — ver normas no repositório.
