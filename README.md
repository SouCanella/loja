...
- Gestão (Fase 3): receitas (margem e preço sugerido na API), produção, relatório (**`GET /api/v1/reports/financial`** — paridade com v2; totais, **`by_product`**, **`by_category`**, **`by_order_status`**, **`period_margin_percent`**); **`/api/v1/inventory-items`** (CRUD de insumos); **`GET /api/v1/me`** inclui `store_slug`, `store_name`, **`vitrine_whatsapp`**, **`store_target_margin_percent`**
- Contrato **DEC-06:** prefixo **`/api/v2`** — envelope `{ "success", "data", "errors" }` em rotas de negócio e públicas; a **app Next.js** (painel, login, vitrine) usa **v2** por omissão — ver [`doc/execucao/api-v1-v2-deprecacao.md`](doc/execucao/api-v1-v2-deprecacao.md)
