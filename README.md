...
- Frontend: `http://localhost:3000` — `/loja/[slug]` vitrine; `/painel` (resumo, link vitrine), `/painel/receitas` (lista + **Produzir lote**), `/painel/receitas/nova`, `/painel/relatorio` (métricas + CSV); `/login`. **WhatsApp (vitrine):** em `stores.theme` use `{"vitrine": {"whatsapp": "+5511…"}}` (ou ajuste via SQL) — ver normas no repositório.
