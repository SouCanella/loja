...
**Referência:** [documento_enterprise.md](../documento_enterprise.md) §6 (modelagem), §10–§11 (receitas e precificação), §17 (API), §19 (fluxos produção/precificação), §22 (MVP — receitas e precificação), §25 (roadmap)  
...
- `POST /api/v1/production` (ordem de produção / execução de receita — desenho exato na implementação).
...
- [ ] Execução de produção atualiza estoque de forma consistente (transacional).
...
Após esta fase, revisar [backlog.md](../backlog.md): marcar MVP como completo ou listar gaps.
