...
**Contexto:** o painel já tem **Relatório financeiro** (`/painel/relatorio`) com agregados por produto/categoria/status e gráficos (Pareto, dispersão margem × volume), e o **dashboard** com série de receita e pedidos por estado. **Analytics de vitrine (MVP):** tabela `vitrine_analytics_events`, `POST /api/v2/public/stores/{slug}/analytics/events`, resumo `GET /api/v2/analytics/vitrine/summary` e página **`/painel/analytics-vitrine`**. Este documento alinha o **pedido de produto** (lista longa de KPIs e secções) com **o que o sistema já pode calcular**, o que exige **novos campos ou tabelas**, e o que é **analytics de vitrine** (eventos), tipicamente noutra camada.
...
| **Analytics de vitrine** | Visitas, visualizações de página, «adicionou ao carrinho», produtos mais vistos, **localização** de visitantes | **Parcial:** eventos no núcleo (`page_view`, `product_view`, `add_to_cart`, `checkout_open`); sessões distintas por `session_id`; país opcional (`CF-IPCountry` / `X-Country-Code`). Geo fina e funil longo continuam em evolução (ver §4). |
...
4. **Eventos vitrine** — MVP entregue na **Fase 3.2** (ingestão + resumo painel); extensões (funil longo, mapas, PRO) em **Fase 4** ou backlog.
...
*Última revisão: 2026-04-20 — MVP analytics vitrine no repositório (eventos + painel); **DEC-22** e [relatorios-definicoes-negocio.md](relatorios-definicoes-negocio.md).*
