...
| Dividido **pagos** vs **pendentes** | **Novo / decisão** | Hoje **não há** campo `pago` ou `payment_settled`. Opções: (a) novo campo `payment_status` ou `paid_at`; (b) *proxy* por estado (ex.: «pendente» = `aguardando_confirmacao`; «confirmado operacionalmente» = `confirmado` em diante) — **DEC a registar** para não distorcer relatórios. |
| Receita por hora | **Já** | Soma de `order_items` no intervalo hora, mesmos filtros de estado que o relatório. |
| Receita **confirmada** vs **pendente** | **Parcial** | Pode mapear-se a **faixas de DEC-14** (ex.: «pendente» = só `aguardando_confirmacao`; «confirmada» = `confirmado`…`entregue`) se o negócio aceitar essa definição. **Não** é «receita do bolo» (custo de receita) — é **receita de vendas** (soma de linhas). |
...
| Descontos — total concedido | **Novo** | Não existe `discount` em `order_items` nem cabeçalho de pedido — **migração** ou fora de âmbito até haver regra de negócio. |
...
| Uso de cupons | **Novo** | Sem entidade cupom no modelo — backlog. |
...
*Última revisão: 2026-04-20 — alinhamento do pedido de relatórios/analytics ampliado ao modelo e à separação vitrine vs operação.*
