...
**Última actualização:** 2026-04-22 — **DEC-24** (precificação alargada: custos adicionais, canais %, fixos faseados, escalões de quantidade — ver tabela e ADR; detalhe em [backlog.md](backlog.md)). **2026-04-20** — **DEC-21** fechada tecnicamente (impressão: JSON + HTML/`window.print`, Web USB experimental — ver ADR abaixo). **DEC-23** reservada (stock opcional por produto, [ideias-compartilhar-cardapio-estoque-por-produto.md](ideias-compartilhar-cardapio-estoque-por-produto.md)). **DEC-22:** relatórios — [relatorios-definicoes-negocio.md](relatorios-definicoes-negocio.md). **DEC-06:** **`/api/v2`** é o contrato usado pelo **Next.js** (painel, login, dados públicos SSR); **`/api/v1`** mantém-se em paridade para testes e integrações. **DEC-16:** refresh via **`POST /api/v2/auth/refresh`** no cliente. Ver [qualidade-e-conformidade.md](qualidade-e-conformidade.md), [api-v1-v2-deprecacao.md](../execucao/api-v1-v2-deprecacao.md). **Vitrine vs mockup:** [paridade-mockup-vitrine.md](paridade-mockup-vitrine.md); **tema / fundo / logo na vitrine:** [vitrine-configuracao-aparencia.md](vitrine-configuracao-aparencia.md); pedidos WhatsApp ↔ painel: **IP-11** em [backlog.md](backlog.md). OpenAPI em `doc/api/openapi.json` (`make openapi-export`).
...
| **DEC-24** | **Precificação alargada (evolução face a RF-PR-01 / planilha):** **(1)** cadastro de **custos adicionais** (embalagem, outros) somados ao custo variável **antes** da margem; **(2)** **canais / apps:** um ou mais campos **%** sobre uma **referência** fixada por norma (ex. preço sugerido ou preço de catálogo) com saída **preço sugerido por canal** — **sem** comissão iterativa «% sobre preço final» na v1; **(3)** **MO** mantém **taxa R$/h** na loja (alinhado a DEC-09 + §3 de [painel-ux-layout-formularios-precificacao.md](painel-ux-layout-formularios-precificacao.md)); **(4)** **custos fixos mensais:** por defeito **só painel informativo** (contribuição / equilíbrio); **opt-in** para incluir no preço unitário com modo **global (mix)** ou **por produto** e **alerta** se o volume previsto for baixo; **(5)** **escalões de quantidade** configuráveis pelo admin (N unidades, D%) para preço em lote / B2B. Implementação **por fases**; prioridades e modelo de dados em [backlog.md](backlog.md) secção **Precificação — evolução**. | Backlog |
...
- **DEC-21:** Impressão combina **dados normalizados no servidor** (`GET /api/v2/orders/{id}/print`, `stores.config.print`) com **renderização no cliente**: recibo em **HTML** e impressão via **janela dedicada + diálogo do sistema** (funciona com laser/jato e com térmicas se o SO/driver as expuserem como impressora normal). **Web USB** envia bytes **ESC/POS** gerados no cliente (**experimental**, Chrome/Edge, HTTPS, filtros por fabricante). **Bluetooth** via browser mantém-se **opcional / baixa compatibilidade**; **agente local** ou app nativo ficam como evolução se o hardware não for coberto. **Consequência:** não prometer suporte a todos os modelos térmicos; matriz de testes em dispositivos reais e fallback sempre disponível (`window.print`).
...
- **DEC-24:** A planilha de custo completo mistura variável, comissão sobre preço, fixo rateado e lote — implementar tudo de uma vez quebra UX e gera preços «saltados» quando um SKU leva o custo fixo inteiro. **Consequência:** custos adicionais e canais % entram primeiro com regra simples; fixos começam como visão global da loja e só depois opt-in com modos e avisos; MO permanece R$/h até haver pedido explícito de salário ÷ horas; escalões de quantidade são dados de configuração, não inferidos. Código e APIs evoluem alinhados a esta ordem no [backlog.md](backlog.md).
...
|------|------------------------|
| **ADR leve** por DEC | Secção **ADR leve** acima (DEC-01 … DEC-24) |
...
Decisões normativas: tabela **Decisões já tomadas** acima (DEC-01 … DEC-24).
